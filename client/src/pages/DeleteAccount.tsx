...
type AppTarget = "igm" | "manager" | "igmhome" | "igmhomemanager" | "hcapp";
...
export default function DeleteAccount() {
  ...
  const igmHomeManagerSteps = [
    {
      number: 1,
      title: "개인정보 보호책임자에게 삭제 요청 이메일 발송",
      description: "아래 이메일 주소로 계정 삭제를 요청하는 이메일을 보내주세요.",
      detail: `이메일 주소: ${policyMeta.privacyOfficer.email}\n\n이메일 제목에 \"[IGM 보호자 앱] 계정 삭제 요청\"을 포함하고, 본문에 가입 시 사용한 아이디와 이름을 기재해 주세요.`,
    },
    {
      number: 2,
      title: "본인 확인 절차 진행",
  ...
  const igmHomeManagerDeletedData = [
    { category: "계정 정보", items: "아이디, 비밀번호, 이름, 이메일, 휴대폰 번호", timing: "요청 후 7일 이내 삭제" },
    { category: "보호자 설정 정보", items: "초대 코드, 자녀 연결 정보, 스트레스 임계값/기준선 설정값, 안심존 설정", timing: "요청 후 7일 이내 삭제" },
    { category: "기기·서비스 정보", items: "기기 ID, 푸시 알림 토큰(FCM), WebSocket 세션 기록", timing: "요청 후 7일 이내 삭제" },
    { category: "열람한 자녀 건강 데이터", items: "심박수, 스트레스 지수, 체온, 위치 기록 등 (자녀 계정 소유 데이터)", timing: "보호자 계정 삭제 후에도 자녀 계정 삭제 시 별도 처리" },
  ];
  ...
  const hcAppSteps = [
    {
      ...
      title: "개인정보 보호책임자에게 삭제 요청 이메일 발송",
      description: "아래 이메일 주소로 계정 삭제를 요청하는 이메일을 보내주세요.",
      detail: `이메일 주소: ${policyMeta.privacyOfficer.email}\n\n이메일 제목에 \"[IGM 건강 케어 앱] 계정 삭제 요청\"을 포함하고, 본문에 가입 시 사용한 아이디와 이름을 기재해 주세요.`,
    },
    {
      number: 2,
      title: "본인 확인 절차 진행",
  ...
  const hcAppDeletedData = [
    { category: "계정 정보", items: "아이디, 비밀번호, 이름, 이메일, 전화번호, 나이", timing: "요청 후 7일 이내 삭제" },
    { category: "건강 프로필", items: "신장, 체중, 복용 중인 영양제, 만성질환, 최근 감기/발열 여부", timing: "요청 후 7일 이내 삭제" },
    { category: "워치 건강 데이터", items: "심박수, HRV, 혈압, 호흡수, 산소포화도(SpO2), 체온, 체지방률, 걸음 수, 피로도", timing: "요청 후 7일 이내 삭제" },
    { category: "음성 챗 기록", items: "음성 챗 사용 시 녹음 파일 및 처리 기록", timing: "요청 후 7일 이내 삭제" },
    { category: "기기·서비스 정보", items: "워치 MAC 주소, 스마트폰 기기 정보, 기기 ID", timing: "요청 후 7일 이내 삭제" },
    { category: "커뮤니티 게시글", items: "질문·리뷰 (기기에만 저장됨)", timing: "앱 삭제 시 기기에서 함께 삭제" },
  ];
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        >
          ...
          {appTarget === "igmhomemanager" && (
            <p className="text-xs text-muted-foreground leading-relaxed mt-3 max-w-2xl">
              IGM 보호자 앱은 자녀의 건강·안전 상태를 모니터링하는 보호자용 앱입니다.
              보호자 계정 삭제 시 자녀의 건강 데이터는 자녀 앱 계정의 삭제 절차에 따라 별도 처리됩니다.
            </p>
          )}
          {appTarget === "hcapp" && (
            <p className="text-xs text-muted-foreground leading-relaxed mt-3 max-w-2xl">
              커뮤니티 게시글(질문·리뷰)은 회사 서버로 전송되지 않고 기기에만 저장됩니다.
              앱을 삭제하시면 기기에 저장된 게시글도 함께 삭제됩니다.
            </p>
          )}
          ...
          <div className="mt-6 inline-flex p-1 bg-muted rounded-lg border border-border" role="tablist" aria-label="앱 선택">
            ...
            <button
              role="tab"
              aria-selected={appTarget === "igmhomemanager"}
              onClick={() => {
                setAppTarget("igmhomemanager");
                setExpandedStep(null);
              }}
              className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                appTarget === "igmhomemanager" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              IGM 보호자 앱
            </button>
            <button
              role="tab"
              aria-selected={appTarget === "hcapp"}
              onClick={() => {
                setAppTarget("hcapp");
                setExpandedStep(null);
              }}
              className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                appTarget === "hcapp" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              IGM 건강 케어 앱
            </button>
