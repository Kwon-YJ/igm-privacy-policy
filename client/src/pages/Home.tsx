/*
 * Design: "Legal Clarity" — Swiss Typography based legal document style
 * - Left fixed sidebar navigation with scroll tracking
 * - Right wide content area with clear section hierarchy
 * - IGM Blue (#4A80F0) for section numbers and links
 * - White background with subtle blue-gray alternating sections
 * - Pretendard font with clear typographic hierarchy
 */

import { useState, useEffect, useRef, useCallback } from "react";
import { policySections, policyMeta } from "@/data/privacyPolicy";
import { Shield, ChevronUp, Menu, X, Trash2 } from "lucide-react";
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-muted-foreground -mt-0.5">개인정보처리방침</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="hidden sm:flex items-center gap-1.5 text-xs text-muted-foreground">
                <Shield className="w-3.5 h-3.5" />
                <span>시행일: {policyMeta.effectiveDate}</span>
              </div>
              {/* Mobile menu button */}
              <button
                ...
                {policySections.map((section) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`font-mono text-[11px] mr-1.5 ${isActive ? "text-primary" : "opacity-50"}`}>
                        {String(section.number).padStart(2, "0")}
                      </span>
                      {section.title}
                    </button>
          ...
          <main className="flex-1 min-w-0 py-8 lg:py-10">
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-semibold text-primary uppercase tracking-wider">Privacy Policy</span>
              </div>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-foreground tracking-tight mb-3">
                개인정보처리방침
              </h2>
              <p className="text-sm text-muted-foreground leading-relaxed max-w-2xl">
                IGM은 「개인정보 보호법」 제30조에 따라 정보주체의 개인정보를 보호하고
              ...
              <div className="flex flex-wrap gap-4 mt-4 text-xs text-muted-foreground">
                <span className="flex items-center gap-1.5">
                  ...
                  시행일: {policyMeta.effectiveDate}
                </span>
                <span>최종 수정일: {policyMeta.lastUpdated}</span>
              </div>
            </motion.div>
            ...
            <div className="space-y-0">
              {policySections.map((section, index) => (
                <motion.section
                  ...
                  className={`py-8 ${
                    index < policySections.length - 1 ? "border-b border-border/60" : ""
                  }`}
                >
            ...
            <div className="mt-12 p-5 sm:p-6 bg-destructive/5 border border-destructive/15 rounded-xl">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-foreground mb-1">계정 및 데이터 삭제 요청</h3>
                  <p className="text-xs text-muted-foreground leading-relaxed mb-3">
                    IGM 앱 계정과 관련 데이터의 삭제를 원하시면 아래 링크에서 삭제 절차를 확인하실 수 있습니다.
                  </p>
                  <Link href="/delete-account" className="inline-flex items-center gap-1.5 text-xs font-semibold text-destructive hover:underline">
                    삭제 요청 페이지로 이동
