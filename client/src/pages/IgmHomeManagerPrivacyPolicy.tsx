 ...
 * Privacy policy page for IGM 보호자 앱 / igm_home_manager
 */

import { useState, useEffect, useRef, useCallback } from "react";
import {
  policySectionsIgmHomeManager,
  policyMetaIgmHomeManager,
} from "@/data/privacyPolicyIgmHomeManager";
...
export default function IgmHomeManagerPrivacyPolicy() {
  const [activeSection, setActiveSection] = useState(
    policySectionsIgmHomeManager[0].id
  );
  ...
  const handleScroll = useCallback(() => {
    ...
    let currentSection = policySectionsIgmHomeManager[0].id;
    for (const section of policySectionsIgmHomeManager) {
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-3">
              <div className="hidden sm:flex items-center gap-1.5 text-xs text-muted-foreground">
                ...
                <span>시행일: {policyMetaIgmHomeManager.effectiveDate}</span>
              </div>
              <button
        ...
        {mobileMenuOpen && (
          ...
          >
            <nav className="p-4 space-y-0.5">
              {policySectionsIgmHomeManager.map((section) => (
                <button
                  key={section.id}
          ...
          <main className="flex-1 min-w-0 py-8 lg:py-10">
            ...
            >
              ...
              <p className="text-sm text-muted-foreground leading-relaxed max-w-2xl">
                ...
                하기 위하여, 자녀의 건강·안전 상태를 모니터링하는 보호자 앱(이하
                "본 앱")에 대하여 다음과 같이 개인정보처리방침을
                수립·공개합니다.
              </p>
              ...
              <div className="flex flex-wrap gap-4 mt-4 text-xs text-muted-foreground">
                <span className="flex items-center gap-1.5">
                  ...
                  시행일: {policyMetaIgmHomeManager.effectiveDate}
                </span>
                <span>최종 수정일: {policyMetaIgmHomeManager.lastUpdated}</span>
              </div>
            </motion.div>

            {/* Sections */}
            <div className="space-y-0">
              {policySectionsIgmHomeManager.map((section, index) => (
                <motion.section
                  key={section.id}
                  ref={(el) => {
                  ...
                  className={`py-8 ${
                    index < policySectionsIgmHomeManager.length - 1
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed mb-3">
                    보호자 앱 계정과 관련 데이터의 삭제를 원하시면 아래 링크에서
                    삭제 절차를 확인하실 수 있습니다.
                  </p>
                  <Link
                    href="/delete-account"
            ...
            <div className="mt-16 pt-8 border-t border-border">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8">
                ...
                <div className="text-xs text-muted-foreground space-y-1">
                  ...
                  <p>
                    대표: {policyMetaIgmHomeManager.representative} |
                    사업자등록번호: {policyMetaIgmHomeManager.businessNumber}
                  </p>
                  <p>
                    이메일: {policyMetaIgmHomeManager.email} | 전화:{" "}
                    {policyMetaIgmHomeManager.phone}
                  </p>
                </div>
              </div>
